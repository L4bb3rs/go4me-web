...
export default function GlobalWalletBar({ inline = false }: { inline?: boolean }) {
  ...
  const {
    isAvailable: _gobyAvailable,
    isConnected: gobyConnected,
    accounts: gobyAccounts,
    connect: _gobyConnect,
  ...
  // Derived flag retained for clarity; prefix with underscore to silence unused warnings if not referenced
  const _isConnectedAny = !!session || (gobyConnected && !isMobile)
  ...
  const walletName = useMemo(() => {
    ...
    // Session typing may vary across versions
    return (session as unknown as { peer?: { metadata?: { name?: string } } })?.peer?.metadata?.name || 'Wallet'
