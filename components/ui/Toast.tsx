...
export function Toast({ message, type, isVisible, onClose, duration = 5000 }: ToastProps) {
  ...
  return (
    ...
    >
      <Icon
        // Icon name is controlled via props; fallback is handled in getIcon
        // Casting to string keeps TypeScript happy without using `any`
        name={String(getIcon())}
        style={{
          color: colors.text,
          fontSize: '18px',
...
export function ToastProvider({ children }: { children: ReactNode }) {
  const [toasts, setToasts] = useState<
    Array<{ id: number; message: string; type: 'error' | 'success' | 'warning' | 'info' }>
  >([])
